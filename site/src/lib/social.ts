...
/**
 * A social platform's presentation. Icons live in code because they are assets,
 * not content; the profile URL itself comes from Keystatic Site Settings so
 * editors can change or remove a profile without a code change.
 */
type SocialPlatform = {
  /** Field on the siteSettings singleton holding this profile's URL. */
  settingsKey: 'instagramUrl' | 'linkedinUrl' | 'linktreeUrl';
  label: string;
  /** SVG path data drawn in a 24x24 viewBox with fill="currentColor". */
  path: string;
  /** Set when the path relies on the even-odd fill rule to punch out holes. */
  fillRule?: 'evenodd';
};

/** Display order of social profiles across the site. */
 ...
 * order. Platforms left blank in the CMS are omitted everywhere.
 */
